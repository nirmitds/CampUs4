...
/**
 * Shows a banner when user's ID is not verified.
 * idVerified: "none" | "pending" | "rejected" | "verified"
 * blockedActions: array of action names to show as blocked
 */
export default function VerifyBanner({ idVerified, blockedActions = [] }) {
  ...
  const configs = {
    none: {
      ...
      btn: "Upload ID Card", btnColor: "#ef4444",
    },
    pending: {
      color: "#fbbf24", bg: "rgba(251,191,36,0.08)", border: "rgba(251,191,36,0.25)",
      icon: "⏳", title: "Verification Pending",
      msg: "Your ID card is under review. Features will unlock once admin verifies it.",
      btn: null,
    },
    rejected: {
      color: "#f87171", bg: "rgba(239,68,68,0.08)", border: "rgba(239,68,68,0.25)",
      ...
      btn: "Re-upload ID", btnColor: "#ef4444",
    },
  };
  ...
  return (
    <div style={{
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{ fontSize: 13, color: "rgba(255,255,255,0.55)", marginBottom: blockedActions.length ? 8 : 0 }}>
          {c.msg}
        </div>
      ...
      {c.btn && (
        <button onClick={() => navigate("/student/profile")}
          style={{ padding: "8px 16px", borderRadius: 10, background: c.btnColor, border: "none", color: "#fff", fontFamily: "Outfit,sans-serif", fontSize: 13, fontWeight: 700, cursor: "pointer", flexShrink: 0 }}>
          {c.btn} →
        </button>
      )}
    </div>
